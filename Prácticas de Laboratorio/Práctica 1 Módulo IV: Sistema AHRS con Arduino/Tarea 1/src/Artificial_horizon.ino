...
{ 
  ...
  println(Serial.list());//[2]); //Shows your connected serial ports
...
void serialEvent(Serial p) //Reading the datas by Processing.
{
  if (p.available() >= 18) {
...
{ 
  ...
  if (hq != null) { // use home quaternion
    ...
    text("Disable home position by pressing \"n\"", 20, 0);
  } else {
    quaternionToEuler(q, Euler);
    textSize(20);
    fill(255);
    text("Point FreeIMU's X axis to your monitor then press \"h\"", -500, -500);
  }
  ...
  fill(255); 
  text("Q:\n" + q[0] + "\n" + q[1] + "\n" + q[2] + "\n" + q[3], -700, -500);
  text("Euler Angles:\nYaw (psi)  : " + degrees(Euler[0]) + "\nPitch (theta): " + degrees(Euler[1]) + "\nRoll (phi)  : " + degrees(Euler[2]), -500, -500);
}
void MakeAnglesDependentOnMPU6050() 
{ 
  Bank = -Euler[2]; 
  Pitch= degrees(Euler[1])*5; 
  Azimuth= degrees(Euler[0]);
}
void Horizon() 
{ 
