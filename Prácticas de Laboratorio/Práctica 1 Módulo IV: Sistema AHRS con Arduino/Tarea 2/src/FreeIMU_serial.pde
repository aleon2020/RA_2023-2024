...
// INCLUIR LA BIBLIOTECA DEL BMP280 AL PRINCIPIO DEL CÓDIGO
...
// DECLARAR UNA MATRIZ LLAMADA BARO PARA ALMACENAR LOS DATOS DEL BARÓMETRO
...
// DECLARAR UN OBJETO DE LA CLASE Adafruit_BMP280
Adafruit_BMP280 bmp;
...
//INICIALIZACIÓN DEL BARÓMETRO
if(!bmp.begin()) {
	Serial.print(F("El sensor no está conectado"));
 	while (1);
}
...
void loop() {
  if(Serial.available()) {
    ...
    // MODIFICAR LA SECCIÓN CORRESPONDIENTE AL COMANDO 'q'
    else if (cmd == 'q') {
      uint8_t count = serial_busy_wait();
      for (uint8_t i = 0; i < count; i++) {
         my3IMU.getQ(q);
         // Obtener datos del barómetro
         my3IMU.getTempPressureAltitude(baro);
         serialPrintFloatArr(q, 4);
         // Enviar datos del barómetro al puerto serie
         serialPrintFloatArr(baro, 3);
         Serial.println("");
       }
    }
...
// FUNCIÓN ENCARGADA DE OBTENER LOS DATOS DEL BARÓMETRO
void getTempPressureAltitude(float baro[3]) {
   baro[0] = bmp.readTemperature();
   baro[1] = bmp.readPressure() / 100.0F;
   baro[2] = bmp.readAltitude(SEALEVELPRESSURE_HPA); 
}
